...
def profile_routes(connection,limiter):
    ...
    def getTableData():
        try:
            ...
            query = f"SELECT q.url, q.topic,q.question,q.level, q.platform,uq.mark_date,CASE WHEN uq.user_id IS NOT NULL THEN TRUE ELSE FALSE END AS completed FROM questions q LEFT JOIN user_questions uq ON q.question_id = uq.question_id AND uq.user_id = '{id}' WHERE( uq.user_id = '{id}' OR uq.user_id IS NULL ) ORDER BY q.topic asc"
